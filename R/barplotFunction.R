...
gene.distribution <- function(pathway, gene, pairs, metabolites){
   ...
   if(!is.data.frame(metabolites) && metabolites == "All"){
   metabolites <- unique(getListNodeFromKGML(computer_path_to_map_folder,pathway)$keggId)
   }
...
srd.distribution <- function(distance){
    ...
    p<-ggplot2::ggplot(distanceData,
       ...
       ggplot2::ggtitle(paste("srd distribution_#total srds=",count_srds,sep=""))+
