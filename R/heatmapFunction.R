...
heatmap <- function(association, pathway,computer_path_to_map_folder, commonNames = TRUE){
    ...
    #cat("heatmap function", computer_path_to_map_folder)
    data.result.ob <- create.data.restul.ob(association,
                                            pathway = c(pathway),
                                            F,
                                            F,
                                            F,
                                            computer_path_to_map_folder=computer_path_to_map_folder)
    ...
    #print(str(df.result))
    #print(length(gene.uniq))
    #print(length(metabolite.uniq))
    ...
    if(commonNames){
        ...
        metabolite.names <- getCommonNames(as.vector(unlist(metabolite.uniq)), "metabolite")
    ...
    # Set heat colors - V1
    #colors <- c("#BD0026","#E31A1C","#FC4E2A","#FD8D3C","#FEB24C","#FED976", "#FFEDA0", "#FFFFCC")

    # https://waldyrious.net/viridis-palette-generator/
    #colors <- c("#fde725","#d2e21b","#a5db36","#7ad151","#54c568","#35b779","#22a884","#1f988b")
    colors <- c("#fde725","#75d054","#3dbc74","#20a386","#238a8d","#2d718e","#39558c")
    ...
    p2 = ggplot2::ggplot(df.heatmap, ggplot2::aes(x = Row, y = Col, fill = Distance),
        ...
        ggplot2::theme(
            ...
            axis.text = ggplot2::element_text(size=13,colour="black",face="plain"),
        ...
        ggplot2::geom_text(label = as.numeric(df.heatmap$Distance, 1),
                           size = 5,ggplot2::aes(x = Row, y = Col))
    print(p2)
