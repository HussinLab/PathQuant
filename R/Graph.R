...
setMethod("df.sub.data.result.numeric" , "data_result",
          function(object, numeric = FALSE){
              ...
              #test3 <- dplyr::filter(df.out,metabolite=="C00328")
              #print(test3)

              if (numeric == TRUE) {
                  df.out <- df.out[complete.cases(df.out[,8]),] #removes metabolites and genes from heatmap that had only srds = NA
              }

              return <- df.out
          })

createGraphFromPathway <- function(pathwayId,computer_path_to_map_folder){

    #print("createGraphFromPathway")
    #cat(computer_path_to_map_folder)

    #test if KGML file was downloaded already
    if(isFileInDirectory(pathwayId,computer_path_to_map_folder) == FALSE) {
        date <- as.character(format(Sys.time(),"%Y_%m_%d"))

        message(paste0("Could not find the map file within the directory you provided, or within the default one. Trying to download the latest version on KEGG database within : ",
                file.path(here::here("Maps",paste0("Version_Downloaded_",date,"/"))),"\n"))

        computer_path_to_map_folder <-  file.path(here::here("Maps",paste0("Version_Downloaded_",date,"/"))) #consider computer_path_to_map_folder was incorrect, force to download within a correct directory, and use it.
        getPathwayKGML(pathwayId);

    }

    # create df for vertices
    nodeDF <- getListNodeFromKGML(computer_path_to_map_folder,pathwayId);
    #write.table(nodeDF,file =paste0("nodes_",pathwayId,".tsv"),row.names = F,sep="\t",quote = F)

    # create df edges
    edgeDF <- finalReactionEdgeDF(computer_path_to_map_folder,pathwayId);
    #write.table(edgeDF,file = paste0("edges_",pathwayId,".tsv"),row.names = F,sep="\t",quote = F)
    ...
    if(!(is.null(nodeDF)) && !(is.null(edgeDF))) {
        ...
        # p <- plot(igraphe,edge.arrow.size=.2, edge.curved=0.1,
        #
        ...
        #just to get vizualisation of graph created
        # cat("Graph is not empty, the pathway id is : ", pathwayId)
        # png(paste0(pathwayId,".png"), width=2800, height=2500, res=200)
        # plot(igraphe,edge.arrow.size=.2, edge.curved=0.1, 
        #
        #      vertex.color="orange", vertex.frame.color="#555555",
        #
        #      vertex.label.color="black",
...
setGeneric("getDistanceAsso", function(object, pathwayId,computer_path_to_map_folder, path)
{
...
setMethod("getDistanceAsso", "data_annotated", function(object,
    ...
    # graph creation
    if (!exists("graphe")) {
        graphe <- createGraphFromPathway(pathwayId,computer_path_to_map_folder);
    }
...
get.srd <- function(pairs,
    ...
    if(dplyr::is.tbl(pairs)){
        message(" get.srd() needs a dataframe as input, your input is a tibble.\n We forced the conversion to dataframe. Forced conversion might lead to unexpected outputs.")
        pairs <- as.data.frame(pairs)
    }

    if(computer_path_to_map_folder=="") {
        date <- as.character(format(Sys.time(),"%Y_%m_%d"))
        computer_path_to_map_folder <-  file.path(here::here("Maps",paste0("Version_Downloaded_",date,"/")))

        message(paste0("Important message : You did not provide path to a map directory. Will search, then try to download the latest version in KEGG database within this directory : \n",
                       computer_path_to_map_folder))
    }


    data.result.ob <- create.data.restul.ob(pairs, organism_code, pathway,
                                            ordered, commonNames, path,computer_path_to_map_folder)
    # object to df
...
create.data.restul.ob <- function(pairs,
    ...
    #cat("create.data.restul.ob",computer_path_to_map_folder)
    ...
    if (length(pathway) == 1 && pathway == "All") {
        ...
        #pathway.list <- pathway.list[-22] #-"hsa00260" ? Answer : this path generates unique errors.
        pathway.list <- pathway.list[!pathway.list %in% c("hsa00260")]
        #print(pathway.list)
    } else {
        #Debug print
        #print("ici")
    ...
    for(i in 1:length(pathway.list)){
        ...
        if (path == FALSE) {
            r <-getDistanceAsso(data.annotated.ob, pathway.list[i],computer_path_to_map_folder, FALSE);

        } else {
            r <-getDistanceAsso(data.annotated.ob, pathway.list[i],computer_path_to_map_folder, TRUE);
        }
        if (is.data.frame(r)) {
...
#     graphe <- createGraphFromPathway("hsa01100",computer_path_to_map_folder);
#     print("allo2")
#     p<- list();
#     for(i in 1:1002){
