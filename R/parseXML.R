...
getListNodeFromKGML <- function(computer_path_to_map_folder,pathwayId) {

    #print("getListNodeFromKGML")

    xmltop <- getKGMLRootNode(computer_path_to_map_folder,pathwayId);
...
getListReactionFromKGML <- function(computer_path_to_map_folder,pathwayId) {

    #print("getListReactionFromKGML")
    #gives content of root
    xmltop <- getKGMLRootNode(computer_path_to_map_folder,pathwayId);
    #capture.output(print(xmltop),file="xxxx_xmltop.txt")
    reactionIdNodes <- XML::getNodeSet(xmltop, "//reaction");
    #capture.output(print(reactionIdNodes),file="xxxx.txt")#test to understand
...
getListOfUniqueHSAGeneId <- function(pathwayId){
    ...
    gene.list <- getListEdgeFromGeneKGML(computer_path_to_map_folder,pathwayId)$ko
...
getListOfUniqueMetabolite <- function(pathwayId){
    ...
    metabo.list <- getListNodeFromKGML(computer_path_to_map_folder,pathwayId)$keggId
...
getListUniqueMetaboliteInReactions <- function(pathwayId){
    ...
    reac.sub.list <- getListReactionFromKGML(computer_path_to_map_folder,pathwayId)$substrateName
    reac.prod.list <- getListReactionFromKGML(computer_path_to_map_folder,pathwayId)$productName
...
getListEdgeFromGeneKGML <- function(computer_path_to_map_folder,pathwayId) {
...
getListOrthologGeneFromKGML <- function(computer_path_to_map_folder,pathwayId) {
...
getKGMLRootNode <- function(computer_path_to_map_folder,pathwayId){
    # get the root of the KGML document
    #Debug print
    #print("getKGMLRootNode")

    pathFile <- toStringPathFile(computer_path_to_map_folder,pathwayId);
...
toStringPathFile <- function(computer_path_to_map_folder,pathwayId){
    ...
    pathFile <- paste0(computer_path_to_map_folder, s2, s3, sep="");
    ...
    if (is.na(keggId)) {
        ...
        while(foundName == FALSE){
            ...
            if(type == "gene"){
                ...
                if(!is.null(tmp[[1]][1])){
                    if(tmp[[1]][1] =="SYMBOL"){ #rather than NAME since Nov 2021, bug was taking RefSeq as name since KEGG update. Fixed using SYMBOL.
